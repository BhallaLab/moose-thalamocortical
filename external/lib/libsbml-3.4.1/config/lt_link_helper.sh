        ...
        *.o)
        ...
        lib*.la | */lib*.la)
            mkdir -p "${stage_dir}/${libdir}" && \
                eval "/data/subha/moose/external/lib/libsbml-3.4.1/libtool --mode=install cp \"$1\" \"${stage_dir}/${libdir}\""
...
eval "/data/subha/moose/external/lib/libsbml-3.4.1/libtool --tag=\"$TAG\" --mode=link \"$LD\" -rpath \"${libdir}\" -inst-prefix-dir \"${stage_dir}\" -module -avoid-version -export-dynamic -shrext "${dllibext}" -o \"${out_dir}/${out_filename}.la\" $args" && \
    eval "/data/subha/moose/external/lib/libsbml-3.4.1/libtool --mode=install cp \"${out_dir}/${out_filename}.la\" \"${stage_dir}/${libdir}/${out_filename}.la\"" 
