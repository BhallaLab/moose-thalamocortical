...
namespace pymoose{
    ...
    class CaConc : public Neutral
    {
      public:
        ...
        CaConc(std::string className, std::string objectName, Id parentId);
        CaConc(std::string className, std::string path);
        CaConc(std::string className, std::string objectName, PyMooseBase& parent);
        CaConc(Id id);
