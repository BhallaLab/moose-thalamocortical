...
namespace pymoose{
    ...
    class AscFile : public Neutral
    {
      public:
        ...
        AscFile(std::string className, std::string objectName, Id parentId);
        AscFile(std::string className, std::string path);
        AscFile(std::string className, std::string objectName, PyMooseBase& parent);
        AscFile(Id id);
