...
namespace pymoose{
    ...
    class NMDAChan : public SynChan
    {
      public:
        ...
        NMDAChan(std::string className, std::string objectName, Id parentId);
        NMDAChan(std::string className, std::string path);
        NMDAChan(std::string className, std::string objectName, PyMooseBase& parent);
        NMDAChan(Id id);
