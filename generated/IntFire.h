...
namespace pymoose{
    ...
    class IntFire : public Neutral
    {
      public:
        ...
        IntFire(std::string className, std::string objectName, Id parentId);
        IntFire(std::string className, std::string path);
        IntFire(std::string className, std::string objectName, PyMooseBase& parent);
        IntFire(Id id);
