...
namespace pymoose{
    ...
    class Adaptor : public Neutral
    {
      public:
        ...
        Adaptor(std::string className, std::string objectName, Id parentId);
        Adaptor(std::string className, std::string path);
        Adaptor(std::string className, std::string objectName, PyMooseBase& parent);
        Adaptor(Id id);
